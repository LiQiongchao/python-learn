...
def parse_call_log():
    ...
    for item in tbody_items:
        ...
        startTime = "20" + td[5].text
        callDict['startTime'] = startTime
        durationStr = td[6].text
        durationMinute = 0
        if (durationStr.find('分') != -1):
            durationArr = durationStr.split('分')
            durationMinute = int(durationArr[0]) * 60
            durationStr = durationArr[1]
        duration = durationMinute + int(durationStr.replace('秒', ''))
        print("du - ", duration)
        callDict['endTime'] = startTime
